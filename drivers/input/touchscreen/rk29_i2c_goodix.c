...
//****************************************************************
static struct rk_ts_data *g_ts;
#define MAX_CONTACTS	5
#define SCREEN_MAX_X	TS_MAX_X
#define SCREEN_MAX_Y	TS_MAX_Y
...
#endif
//*---------------------------------------------------------------
//* move from rk29_i2c_goodix.h to here
...
//***************************************************************
	...
	input_mt_init_slots(ts->input_dev, MAX_CONTACTS);
...
//* Add by zbiao for android box
//****************************************************************
	if(goodix_input_params_init(ts) != 0)
		printk("^^^^^^^^^ %s:goodix_input_params_init failed !!!!!!\n", __FILE__);
	else{
		printk("^^^^^^^^^ %s:goodix_input_params_init successed !!!!!!\n", __FILE__);
		g_ts = ts;
	}
	goto bonovo_exit;
//****************************************************************
...
//* Add by zbiao for android box
//****************************************************************
void ft5x0x_report_bonovo_value(struct ts_event *event)
{

	int i;
	//printk(KERN_INFO "----%s ts_event->touch_point=%d ----\n",__func__,event->point.id);
	struct rk_ts_data *data;
	if(g_ts == NULL){
		printk("++++++++ g_ts is NULL\n");
		return;
	}
	data = i2c_get_clientdata(g_ts->client);
	
	bonovo_printk("--------- touch_point:%d\n", event->touch_point);

	for(i=0; i<event->touch_point; i++)
	{
		if(event->point[i].status == 1)
		{
			...
			bonovo_printk("--------- x:%d y:%d\n", event->point[i].x, event->point[i].y);
		}
		else if(event->point[i].status == 0)	//up event
		{
			input_mt_slot(data->input_dev, event->point[i].id);
			input_report_abs(data->input_dev, ABS_MT_TRACKING_ID, -1);
		}
		printk("=========Touch point ID: %d, status: %d, Xpos: %d, Ypos: %d\r\n", 
			event->point[i].id, event->point[i].status, 
			event->point[i].x, event->point[i].y);
	}
	input_sync(data->input_dev);
}	/*end ft5x0x_report_bonovo_value*/
//****************************************************************

// dzwei, 2014-8-7
#define MAX_FINGER_NUM_SUPPORTED	5
void ft5x0x_report_bonovo_touch_event(unsigned char *buf, unsigned int buf_len)
{
	int i;
	int idx;
	struct rk_ts_data *data;
	...
	//---------------------------------------------------------------
	// check buf data validation
	if (!buf)
	{
		return;
	}
	// buf[0] is finger number. If no finger number, just return.
	if (buf[0] == 0)
	{
		return;
	}
	// not all finger info are included in the buffer
	if ((buf[0]*7+1)>buf_len)
	{
		return;
	}
	//---------------------------------------------------------------
	...
	for (i=0; i<buf[0]; i++)
	{
		input_mt_slot(data->input_dev, buf[idx]);		// 
		if (buf[idx+1])		// touch down,
		{
			input_report_abs(data->input_dev, ABS_MT_TRACKING_ID, buf[idx]);	//ID
			input_report_abs(data->input_dev, ABS_MT_POSITION_X, buf[idx+2]+(buf[idx+3]<<8));
			input_report_abs(data->input_dev, ABS_MT_POSITION_Y, buf[idx+4]+(buf[idx+5]<<8));
		}
		else
		{
			input_report_abs(data->input_dev, ABS_MT_TRACKING_ID, -1);
		...
		//printk("=========Touch point ID: %d, status: %d, Xpos: %d, Ypos: %d\r\n", 
		//	buf[idx], buf[idx+1], buf[idx+2]+(buf[idx+3]<<8), buf[idx+4]+(buf[idx+5]<<8));
		idx += 7;	// the information length of one finger is 7 bytes
	}
	input_sync(data->input_dev);
}
