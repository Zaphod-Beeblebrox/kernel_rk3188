...
usbnet_probe (struct usb_interface *udev, const struct usb_device_id *prod)
{
	...
	dev->net = net;
	//---------------------------------------------------------------
	// dzwei, 2014-7-9, sync the code from rockchips
	if((le16_to_cpu(xdev->descriptor.idVendor) == 0x19d2) && 
	(le16_to_cpu(xdev->descriptor.idProduct) == 0x1405)){ 
	strcpy (net->name, "eth%d");
	printk("this is zte ncm ......");
	}else{ 
